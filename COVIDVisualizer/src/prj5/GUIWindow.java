...
public class GUIWindow {
     ...
     * 
     * @param statesList
     *            A list of states to be displayed in the window
     */
    ...
    public void drawInfo() {
        ...
        for (int i = 0; i < currentState.getList().size(); i++) {
            ...
            if (current.calculatePercentage() == -1.0) {
                TextShape nullLabel = new TextShape(x, y + height + 10, "NA");
                TextShape label = new TextShape(x, y + height + 35, current
                    .getEthnicityName());
                // center null label text ("NA" text)
                nullLabel.moveTo(x + label.getWidth() / 2 - nullLabel.getWidth()
                    / 2, y + height + 10);
                ...
                TextShape label = new TextShape(x, y + height + 10, current
                    .getEthnicityName());
                DecimalFormat df = new DecimalFormat("#.#");
                TextShape percentage = new TextShape(x, y + height + 35, df
                    .format(current.calculatePercentage()) + "%");
